...
class EditalAnalyzer {
  ...
  Future<Map<String, dynamic>?> gerarPlanoEstudo(String cargoAlvo, Map<String, dynamic> dadosCargo) async {
    try {
      _reportProgress(0.1, 'Gerando plano de estudos para o cargo: $cargoAlvo...');

      // Chamar a API para gerar o plano de estudos
      final String? resultado = await iaService.gerarPlanoEstudos(cargoAlvo: cargoAlvo, dadosCargo: dadosCargo);

      if (resultado == null || resultado.isEmpty) {
        _log('Resultado da geração de plano de estudos vazio');
        return null;
      }

      _reportProgress(0.7, 'Processando resultado da geração...');
      _log('Resultado da geração de plano de estudos recebido. Processando resposta...');

      // Processar o resultado (preferencialmente JSON)
      return _processarRespostaLLM(resultado);
    } catch (e, stackTrace) {
      _log('Erro na geração de plano de estudos: $e\nStackTrace: $stackTrace');
      rethrow;
    }
  }
  ...
  DadosExtraidos _converterParaDadosExtraidos(Map<String, dynamic> dadosJson) {
    try {
      ...
      return DadosExtraidos.fromJson(dadosJson);
    } catch (e, stackTrace) {
