...
class PdfScannerDetector {
  ...
  static Future<double> _isPageScanned(PdfDocument document, int pageIndex) async {
    try {
      ...
      final PdfBitmap bitmap = await _renderPageAsBitmap(page);
  ...
  /// Renderiza uma página como bitmap para análise
  static Future<PdfBitmap> _renderPageAsBitmap(PdfPage page) async {
    ...
    // Criar bitmap
    final PdfBitmap bitmap = PdfBitmap(Uint8List(targetWidth * targetHeight * 4));
  ...
  static Future<double> _analyzeImageForScanFeatures(PdfBitmap bitmap) async {
  ...
  /// Converte um PdfBitmap para o formato da biblioteca image
  static img.Image? _convertPdfBitmapToImage(PdfBitmap bitmap) {
    try {
      ...
      for (int x = 10; x < grayscale.width - 10; x += 20) {
        ...
        for (int dy = -5; dy <= 5; dy++) {
          for (int dx = -5; dx <= 5; dx++) {
            final int pixel = grayscale.getPixel(x + dx, y + dy);
            values.add(img.getBlue(pixel)); // Em escala de cinza, todos os canais são iguais
          }
        }
  ...
  static double _calculateEdgeScore(img.Image image) {
    ...
    for (int y = 0; y < edges.height; y++) {
      for (int x = 0; x < edges.width; x++) {
        final int pixel = edges.getPixel(x, y);
        final int value = img.getLuminance(pixel);
    ...
    for (int y = 0; y < image.height - 8; y += 8) {
      for (int x = 0; x < image.width - 8; x += 8) {
        ...
        for (int dx = 0; dx < 8; dx++) {
          final int top = img.getLuminance(image.getPixel(x + dx, y));
          final int bottom = img.getLuminance(image.getPixel(x + dx, y + 7));
          final int topNext = img.getLuminance(image.getPixel(x + dx, y - 1 < 0 ? 0 : y - 1));
          final int bottomNext = img.getLuminance(image.getPixel(x + dx, y + 8 >= image.height ? image.height - 1 : y + 8));
        ...
        if (!hasBlockEdge) {
          for (int dy = 0; dy < 8; dy++) {
            final int left = img.getLuminance(image.getPixel(x, y + dy));
            final int right = img.getLuminance(image.getPixel(x + 7, y + dy));
            final int leftNext = img.getLuminance(image.getPixel(x - 1 < 0 ? 0 : x - 1, y + dy));
            final int rightNext = img.getLuminance(image.getPixel(x + 8 >= image.width ? image.width - 1 : x + 8, y + dy));
