...
class EditalAnalyzer {
  ...
  String _corrigirYamlMalformado(String yamlStr) {
    ...
    String corrigido = yamlStr;

    // Corrigir problemas de indentação
    corrigido = corrigido.replaceAll(RegExp(r'^\s*-\s+([^:]+):\s*$', multiLine: true), '- "$1":');

    // Corrigir strings não escapadas
    corrigido = corrigido.replaceAll(RegExp(r':\s*([^"\'\n\[\]{}:,]+)\s*$', multiLine: true), ': "$1"');

    // Corrigir listas malformadas
    corrigido = corrigido.replaceAll(RegExp(r'^\s*-\s+([^"\'\n\[\]{}:,]+)\s*$', multiLine: true), '- "$1"');
