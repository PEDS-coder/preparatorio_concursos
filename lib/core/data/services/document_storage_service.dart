...
class DocumentStorageService extends ChangeNotifier {
  ...
  Future<void> _initDirectories() async {
    ...
    try {
      ...
      final uploadDir = Directory('${appDir.path}/$_uploadedDocsDir');
      final generatedDir = Directory('${appDir.path}/$_generatedFilesDir');
  ...
  Future<StoredDocument> addUploadedDocument(
    ...
    if (!kIsWeb) {
      final appDir = await getApplicationDocumentsDirectory();
      final file = File('${appDir.path}/$_uploadedDocsDir/$id.$fileType');
  ...
  Future<GeneratedFile> addGeneratedFile(
    ...
    if (!kIsWeb) {
      final appDir = await getApplicationDocumentsDirectory();
      final file = File('${appDir.path}/$_generatedFilesDir/$id.$fileType');
