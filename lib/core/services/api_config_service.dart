  ...
  Future<bool> verificarConfiguracao() async {
    ...
    try {
      ...
      if (_iaService!.isConfigured && _iaService!.apiKey == apiKey && _iaService!.apiType == apiType) {
        print('API LLM já está configurada e válida');
      ...
      print('Verificando validade da API key: $apiType');
      ...
      if (!_isLlmConfigured) {
        ...
        print('API LLM inválida: ${_configErrorMessage}');
      } else {
        print('API LLM validada com sucesso');

        // Garantir que a chave seja salva nas preferências
      ...
      _isVerifyingConfig = false;
      print('Erro na verificação da API LLM: $e');
      notifyListeners();
      return false;
    }
  }
